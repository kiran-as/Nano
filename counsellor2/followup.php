...
error_reporting(-1);
	...
	$studentSql = mysql_query("Select 
	 ...
	 from tbl_rvstudent as a, 
	 tbl_pgdipcourses as b,
	 tbl_councellor as c
	 where a.pgdip_coursename=b.idpgdipcourses 
	 and c.idcouncellor = a.idcouncellor
	 and a.step=2");
...
while($row = mysql_fetch_assoc($studentSql))
{
    ...
    $studentArray[$i]['assignedto'] = $row['firstname'].' '.$row['lastname'];
...
{
   $councellorSql = mysql_query("Select a.review_status,a.created_date,a.councelling_date , b.firstname,b.lastname, c.reviewname
                              from tbl_rvstudentcouncellor as a, 
                              tbl_councellor as b, 
                              tbl_reviewstatus as c
                              where a.councellor_id = b.idcouncellor 
                              and a.review_status = c.idreviewstatus
                              and a.idstudent=$id and created_date !='' order by idrvstudentcouncellor desc limit 0,1");
$i=0;
...
<th>Next Counselling date</th>
				...
				<?php for($i=0;$i<count($studentArray);$i++){
					...
					<tr style="background-color:<?php echo $bgcolor;?>">
						...
						<td><?php echo date('d-M-Y',strtotime($studentArray[$i]['councelling_date']));?></td>
