...
$i=0;
$selectSql = mysql_query("Select * from tbl_paymentdetails where idrvstudent='$studentId'");
while($row = mysql_fetch_assoc(	$selectSql)){
	$paymentResult[$i]['amount'] = $row['amount'];
	$paymentResult[$i]['payment_date'] = date('Y-m-d',strtotime($row['payment_date']));
	$paymentResult[$i]['receipt_date'] = date('Y-m-d',strtotime($row['receipt_date']));
...
$target_file_data = 'applicationPaymentDetails.php'.$sql.'---'.$udpateSql;
...
<div class="form-horizontal  col-sm-6">
		    ...
		    <div class="container">
		    <div class="form-group">
		    		    <h3 class="mar-b20">Application Payment Details</h3>
