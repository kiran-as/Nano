  ...
  if(!empty($searchBox))
  {
      ...
      if($flexiblefield=='With')
      {
          $sql = "rvvlsiid like'%$searchBox%' OR placeofbirth like'%$searchBox%'
           OR fathername like '%$searchBox%' OR mobile like '%$searchBox%'";
      }
      else
      {
          $sql = "rvvlsiid not like '%$searchBox%' OR placeofbirth not like'%$searchBox%'
           OR fathername not like '%$searchBox%' OR mobile not like '%$searchBox%'";

      }

     $studentSql = mysql_query("Select * from 
      tbl_student where ($sql) and idstudent in ($idStudentSelected)");
