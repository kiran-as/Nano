...
while($row = mysql_fetch_assoc($studentSql))
{
    ...
    $studentArray[$i]['idrecruitementresumes'] = $row['idrecruitementresumes'];    
...
function fnUnTag(idrecruitementresumes,idRecruitment){
  cnf = confirm('Do you really want to untag the Student');
  if(cnf==true)
  {
    $('#loadingDiv').show();
  formData = 'idrecruitementresumes=' + idrecruitementresumes;
    $.ajax({
        url: "ajax/ajax_removeStudentTaging.php",
        type: "POST",
        data: formData,
        success: function (data, textStatus, jqXHR)
        {
          $('#loadingDiv').hide();
          parent.location = 'viewRecruitementlistCandidates.php?idrecruitement='+idRecruitment;
...
<!-- Bootstrap core CSS -->
 ...
 <?php }?>
          ...
          $idrecruitementresumes = $studentArray[$i]['idrecruitementresumes'];
          ?>
					<tr>
            ...
            <td><a href='downloadResume.php?idstudent=<?php echo $idstudent;?>' target='_blank'>Download Resume</a></td>
            <td><a href="javascript:;" onclick="fnUnTag(<?php echo $idrecruitementresumes;?>,<?php echo $idRecruitment;?>)">Untag</a>
					</tr>
					<?php }?>
