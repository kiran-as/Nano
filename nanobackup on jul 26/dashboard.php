...
$queryempfactor1 = "SELECT a . * , b . * 
FROM  rv_registration AS a, rv_empfactor AS b
WHERE b.empfactor >=50
AND b.empfactor <=60
AND a.m_id = b.mid
GROUP BY a.m_id"; 
	...
	while ($row = mysql_fetch_assoc($resultc)) {
		...
		$queryempfactor2 = "SELECT a . * , b . * 
FROM  rv_registration AS a, rv_empfactor AS b
WHERE b.empfactor >=61
AND b.empfactor <=70
AND a.m_id = b.mid
GROUP BY a.m_id"; 
	...
	while ($row = mysql_fetch_assoc($resultc)) {
		...
		$queryempfactor3 = "SELECT a . * , b . * 
FROM  rv_registration AS a, rv_empfactor AS b
WHERE b.empfactor >=71
AND b.empfactor <=80
AND a.m_id = b.mid
GROUP BY a.m_id"; 
	...
	while ($row = mysql_fetch_assoc($resultc)) {
		...
		$queryempfactor4 = "SELECT a . * , b . * 
FROM  rv_registration AS a, rv_empfactor AS b
WHERE b.empfactor >=81
AND b.empfactor <=90
AND a.m_id = b.mid
GROUP BY a.m_id"; 
	...
	while ($row = mysql_fetch_assoc($resultc)) {
		...
		$queryempfactor5 = "SELECT a . * , b . * 
FROM  rv_registration AS a, rv_empfactor AS b
WHERE b.empfactor >=91
AND b.empfactor <=100
AND a.m_id = b.mid
GROUP BY a.m_id"; 
	...
	while ($row = mysql_fetch_assoc($resultc)) {
		...
		$queryempfactor6 = "SELECT a . * , b . * 
FROM  rv_registration AS a, rv_empfactor AS b
WHERE b.empfactor >=101
AND a.m_id = b.mid
GROUP BY a.m_id"; 
	$resultc = mysql_query($queryempfactor6);
	$empfactor6=0;
	while ($row = mysql_fetch_assoc($resultc)) {
...
<tr>
<td width="35%" valign="top"><iframe src="ptest/test.php" width="100%" height="260px" border="0" scrolling="no" style="border:0px;"></iframe></td>
...
<div>
		...
		<td width="35%" valign="top"><iframe src="ptest/courses.php" width="100%" height="260px" border="0" scrolling="no" style="border:0px;"></iframe></td>
</tr>
</table>
...
<div class="pullup"><a href="#" id="pullup"><img src="images/pullup.png" id="imgpullup"></a></div>
 ...
 <table width="100%" border="0"  cellpadding="0" cellspacing="0">
	 <tr>
		 <td class="pageheaders" nowrap="nowrap" width="33%">Job Posting for " <?php echo $_SESSION["r_company"];?>"<br>
		 <span class="smalltext" style="font-weight:normal">Click on the "Job Title" to view assigned students resumes</span></td>
		 <td width="33%" align="center"><a href="briefsteps.php" rel="lyteframe" rev="width:450px; height:300px; scrolling:no" align="center">5 easy steps to hire the best talent</a></td>
		 <td width="33%" align="right"><a href="addjobposting.php" class="blueButton" style="margin-top: 5px;">Post New Job Opening</a></td>
	 </tr>
 </table>
...
<div class="row">
<?php if($cntrows==0){?>
<table class="nojobpostings" cellpadding="2" cellspacing="2" border="0" width="100%"><tr><td><img src="images/icon_jobboard.jpg" align="absmiddle" width="190" height="103">You have not created any Job Postings, Please <a href="addjobposting.php">click here </a> to create one.</td></tr></table>
...
echo $curdates;
echo $arrajobpostings[$s]['jp_expdate'];
echo "<br/>";
	 */
	 $processing=0;
	 if($arrajobpostings[$s]['jp_expdate']=='')
	 {
	   $processing=1;
	 }
	 if($processing==0)
	 {
		 if($curdates >= $arrajobpostings[$s]['jp_expdate'])
		 {
			$processing=2;
		 }
	 }
