	...
	while ($rows = mysql_fetch_assoc($resultrecruiter)) {
		...
		if($_POST)
		{
		    ...
		    for($crs=0;$crs<count($_POST['coursess']);$crs++)
               ...
               $degreecgpa = mysql_query("Select a.m_id from rv_educational_details as a
				...
				if($_POST['education'][3]=='')
				 ...
				 $masterquery = mysql_query("Select a.m_id from rv_educational_details as a
		 						   where a.e_percentage >=$master and a.qua_id=1 
		 						   AND a.e_branch in($branchid) AND a.e_course in ($branchsid)");
				 $degreearray = array();			  
				 $resultsdegree=0;
               ...
               $masterquery = mysql_query("Select a.m_id from rv_educational_details as a
		 						   where a.e_percentage >=$mastercgpa and a.qua_id=1  and a.e_percentage_type=2
		 						   AND a.e_branch in($branchid) AND a.e_course in ($branchsid)");
				 $degreearray = array();			  
				 $resultsdegree=0;
				 while ($row = mysql_fetch_assoc($masterquery)) {
				 ...
				 $masterquery = mysql_query("Select a.m_id from rv_educational_details as a
		 						   where a.e_percentage >=$master and a.qua_id=21 and a.e_percentage>0
		 						   AND a.e_branch in($branchid) AND a.e_course in ($branchsid)");
				 $degreearray = array();			  
				 $resultsdegree=0;
				 while ($row = mysql_fetch_assoc($masterquery)) {
				 ...
				 (mid, sessionid)VALUES ('".$midsss."','".$sessionid."')");
				    ...
				    $masterquery = mysql_query("Select a.m_id from rv_educational_details as a
		 						   where a.e_percentage >=$mastercgpa and a.qua_id=21  and a.e_percentage>0 and a.e_percentage_type=2
		 						   AND a.e_branch in($branchid) AND a.e_course in ($branchsid)");
				 $degreearray = array();			  
				 $resultsdegree=0;
				 while ($row = mysql_fetch_assoc($masterquery)) {
