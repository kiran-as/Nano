	...
	while ($rows = mysql_fetch_assoc($resultrecruiter)) {
		...
		if($_POST)
		{
		    ...
		    for($crs=0;$crs<count($_POST['coursess']);$crs++)
		       ...
		       /// print_R($branchsid);//all courses ids
				...
				if($_POST['education'][2]=='')
				 ...
				 (mid, sessionid)VALUES ('".$midsss."','".$sessionid."')");	
				    ...
				    $tenth = mysql_query("Select a.m_id from rv_educational_details as a where  a.qua_id=2 and a.e_percentage >=$degreepercgpa and a.e_percentage_type=2 and a.e_passout_year>='$year' group by a.m_id");
				 $tentharray = array();			  
				 //$resultstenth=0;
				 while ($row = mysql_fetch_assoc($tenth)) {
	    		 $midsss	= $row["m_id"];
               ...
               $degreecgpa = mysql_query("Select a.m_id from rv_educational_details as a
				...
				if($_POST['education'][3]=='')
				 ...
				 $masterquery = mysql_query("Select a.m_id from rv_educational_details as a
		 						   where a.e_percentage >=$master and a.qua_id=1 and a.m_id in ($resultsmd)
		 						   AND a.e_branch in($branchid) AND a.e_course in ($branchsid)");
				 $degreearray = array();			  
				 $resultsdegree=0;
               ...
               $masterquery = mysql_query("Select a.m_id from rv_educational_details as a
		 						   where a.e_percentage >=$mastercgpa and a.qua_id=1  and a.m_id in ($resultsmd) and a.e_percentage_type=2
		 						   AND a.e_branch in($branchid) AND a.e_course in ($branchsid)");
				 $degreearray = array();			  
				 $resultsdegree=0;
				 while ($row = mysql_fetch_assoc($masterquery)) {
				 ...
				 $masterquery = mysql_query("Select a.m_id from rv_educational_details as a
		 						   where a.e_percentage >=$master and a.qua_id=21 and a.m_id in ($resultsmd) and a.e_percentage>0
		 						   AND a.e_branch in($branchid) AND a.e_course in ($branchsid)");
				 $degreearray = array();			  
				 $resultsdegree=0;
				 while ($row = mysql_fetch_assoc($masterquery)) {
				 ...
				 (mid, sessionid)VALUES ('".$midsss."','".$sessionid."')");
				    ...
				    $masterquery = mysql_query("Select a.m_id from rv_educational_details as a
		 						   where a.e_percentage >=$mastercgpa and a.qua_id=21 and a.m_id in ($resultsmd) and a.e_percentage>0 and a.e_percentage_type=2
		 						   AND a.e_branch in($branchid) AND a.e_course in ($branchsid)");
				 $degreearray = array();			  
				 $resultsdegree=0;
				 while ($row = mysql_fetch_assoc($masterquery)) {
				...
				echo $tempsearch;*/
				  ...
				  if($degreepresent==1 && $masterpresent==1)
				 {
				     $finalsql = "Select a.mid as m_id from tbl_temptenth as a,
											   ...
											   tbl_tempdegree as c,
											   tbl_tempmaster as d
							where a.mid=b.mid and a.mid=c.mid and b.mid=c.mid and b.mid=d.mid and a.mid=d.mid and a.sessionid='$sessionid'";
						$arrresultmids= mysql_query($finalsql);
						$resultsmid=0;
						while ($row = mysql_fetch_assoc($arrresultmids)) {
						$midsss	= $row["m_id"];
				...
				$rvvlsi=1;
				  ...
				  if($searchresult!='')
				  {
				    $searchresult= $searchresult.'$';
				     $searchlistarray = explode('$',$searchresult);
					 //print_R($searchlistarray);
					
					 $totalcount = (count($searchlistarray)-1);
					 ///echo $totalcount;
					 for($i=0;$i<$totalcount;$i++)
					 {
					    //echo $i;
						//echo $searchresultindex;
					   $searchresultindex = $searchlistarray[$i];
					    $larresultsdetailss = "SELECT a .m_id 
				                             from rv_registration  as a
							                 LEFT JOIN rv_academic_projects As c ON c.m_id = a.m_id
									        where 
													a.m_id in ($resultssqlsearchmid) 
													AND (
											   a.m_emailid LIKE  '%$searchresultindex%' || 
											   a.m_skills like '%$searchresultindex%' || 
											  a.m_resume_id like '%$searchresultindex%' || 
											  a.m_fname like '%$searchresultindex%' || 
											  c.p_tools like '%$searchresultindex%' || 
											   c.p_challenges like '%$searchresultindex%' || 
												c.p_other_tech like '%$searchresultindex%' || 
											   a.m_skills LIKE '%$searchresultindex%' )
											  Group by a.m_id";
											  //echo $larresultsdetailss;
					   $resultc = mysql_query($larresultsdetailss);
						while ($rowsss = mysql_fetch_assoc($resultc)) 
						{
						  $finalmid = $finalmid.','.$rowsss["m_id"];
						}
						//echo $finalmid;
					 }
				  }
				  ...
				  $finalmidgg=0;
				     $searchresultindex = '';
					   $larresultsdetailss = "SELECT a .m_id  as midss
				                             from rv_registration  as a
									   	     LEFT JOIN rv_academic_projects As c ON c.m_id = a.m_id
									        where 
													a.m_id in ($resultssqlsearchmid) 
											  ...
											  Group by a.m_id";
                      //  echo 	$larresultsdetailss;				  
					  $resultcs = mysql_query($larresultsdetailss);
						while ($rowaa = mysql_fetch_assoc($resultcs)) 
						{
						 //  print_R($rowaa);
						  $finalmid = $finalmid.','.$rowaa['midss'];
						}
				  }
				 // print_R($finalmid);
				if($rvvlsi==1)
...
FROM rv_registration AS a
 JOIN rv_members_ids AS b ON a.m_id = b.m_id where a.m_id in ($finalmid) and a.m_id>200  Group by a.m_id";
					}
				else
					{
				    $larresultsdetails = "SELECT a . * 
                              FROM rv_registration AS a 
									   where a.m_id in ($finalmid) and a.m_id>200 Group by a.m_id";
...
parent.location="advancesearchforjobpostingcombined.php?idjobposting="+jobposting+"&idrecruiter="+idrecruiter+"";
}

function fnvalidate()
{
  var crsv=document.getElementById('coursess').value;
  if(crsv=='')
  {
 ...
 <table>
    ...
    <tr style="background-color:#FFFFFF;">
      <td valign="top"><table width="100%" border="0" cellspacing="2" cellpadding="2">
        ...
        <tr>
          <td> 
						<select multiple="multiple"  id="coursess"  name="coursess[]" class="label"  style="width:150px;height:150px;"> 
...
Show only RV-VLSI Students</td>
      ...
      <td valign="top"><input type="text" name="search" id="search" placeholder="Enter Search word" ><br>
	  <span style="font-size: 10px;color:#EFFEFEF;">Search here by First Name OR Last name OR City OR Skills OR Resume ID <br>OR Email ID OR Tools OR Other Technologies </span></td>
    </tr>
    <tr style="background-color:#FFFFFF;">
...
<input type="submit"  value="Search" name="Search" id="Search" style="background-color:#424843;color:#ffffff;" onclick="return fnvalidate();"></input><a href="searchforjobposting.php?idjobposting=<?php echo $idjobposting;?>&idrecruiter=<?php echo $idrecruiter;?>">Cancel</a></td>
