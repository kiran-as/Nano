...
$_SESSION['idjobposting'] = $idjobposting = $_GET['idjobposting'];
	...
	while ($rows = mysql_fetch_assoc($resultrecruiter)) {
		...
		if($_POST)
		{
			...
			if($rvvlsi==1)
			{
				$larrselect = "SELECT a.*,b.* from rv_registration as a,rv_members_ids as b where a.m_id=b.m_id ";
...
LEFT JOIN rv_members_ids AS b ON a.m_id = b.m_id ,rv_academic_projects As c where a.m_id in ($resultsmid) ";
		 	    $arraStudentss=array();
