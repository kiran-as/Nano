 ...
 $query = "SELECT * FROM $members_table where m_id='".$_SESSION[m_id]."'"; 
  ...
  if(isset($_POST[submit10])){
	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
	    ...
	    header("Location:educations_info.php?msg=1#tabs-1");
	  }
	  ...
	  $db->insertRecord("update  $education_table  set 
	     ...
	     header("Location:educations_info.php?msg=2#tabs-1");
	  }
...
if(isset($_POST[submit12])){
	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
	    ...
	    header("Location:educations_info.php?msg=1#tabs-2");
	  }
	  ...
	  $db->insertRecord("update  $education_table  set 
	     ...
	     header("Location:educations_info.php?msg=2#tabs-2");
	  }
...
if(isset($_POST[GradSubmit])){
	...
	else
	{
	 ...
	 $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
	   ...
	   header("Location:educations_info.php?msg=1#tabs-3");
	  }
...
if(isset($_POST[updateGrad])){
	...
	else
	{
	 ...
	 $db->insertRecord("update  $education_table  set 
	     ...
	     header("Location:educations_info.php?msg=2#tabs-3");
	  }
...
if(isset($_POST[PGSubmit])){
	...
	else
	{
	  ...
	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
	    ...
	    header("Location:educations_info.php?msg=1#tabs-4");
	  }
...
if(isset($_POST[updatePG])){
	...
	else
	{
	  ...
	  $db->insertRecord("update  $education_table  set 
	     ...
	     header("Location:educations_info.php?msg=2#tabs-4");
	  }	  
if(isset($_POST[PGDipSubmit])){
	...
	else
	{
	  ...
	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
															...
															e_institute		='".$ch->checkFields($_POST[selInstPGDip]) ."',
	    ...
	    header("Location:educations_info.php?msg=1#tabs-5");
	  }
...
if(isset($_POST[updatePGDip])){
	...
	else
	{
	  ...
	  $db->insertRecord("update  $education_table  set 
	     ...
	     header("Location:educations_info.php?msg=2#tabs-5");
	  }	  
...
if(isset($_POST[OtherSubmit])){
	  ...
	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
	    ...
	    header("Location:educations_info.php?msg=1#tabs-6");
	  }
...
if($_REQUEST[action]=='deleteOne' && is_numeric($_REQUEST[e_id])!=''){
 ...
 header("Location:educations_info.php?msg=3#tabs-6");
}
...
<form name="formPersonal" id="formPersonal"  action="" method="post" onsubmit="return general_Validation();">
       ...
       <div class="nanohdmid"><a href="educations_info.php" style="text-decoration:none;"><img src="images/step3_bgn.png" width="84" height="24" align="absmiddle" border="0" style=" margin-top:8px;" /></a></div>
       ...
       <div class="nanohdleftdefault"></div>
       <div class="nanohdmiddefault">Step 5</div>
       <div class="nanohdrightdefault"></div>
       </li></a>
       </ul>
...
<div class="demo" style="position:relative; float:left; top:20px; height:500px; margin-left:0px;">
...
<div id="tabs">
    ...
    <?  $results10=$db->getResults("select * FROM $education_table where m_id='".$_SESSION[m_id]."' and qua_id='5' limit 1"); ?>
		<p><strong>Class 10th Details</strong>
        <form name="form10" id="form10" method="post" action="" onsubmit="return class10thvalidation();">
        <table width="100%" cellpadding="3" cellspacing="0" border="0">
		  <tr>
		    <td>&nbsp;</td>
		    <td class="error"><?=messaging($_REQUEST[msg])?></td>
		    </tr>
		  <tr>
		    <td width="21%">Passed Out </td>
		    <td width="79%"><select name="selMonth10" id="selMonth10" style="width:80px;"> 
           
            <? for($m=0;$m<count($month_array);$m++){?>
            <option value="<?=$m?>"  <?=$results10[0]->e_passout_month==$m?'selected':''?>><?=$month_array[$m]?></option>
           
            <? }?></select> 
            &nbsp;&nbsp;&nbsp;&nbsp;<select name="selYear10" id="selYear10" style="width:80px;">
            <option value="0">Year</option>
            <? for($y=1990;$y<=date(Y);$y++){?>
            <option value="<?=$y?>"  <?=$results10[0]->e_passout_year==$y?'selected':''?>><?=$y?></option>
            
            <? }?></select><span><font color="#FF0000" style="margin:125px;">*</font></span></td>
		    </tr>
            <input type="hidden" name="hidId10" value="<?=$results10[0]->e_id?>" />
		  <tr>
		    <td>Aggregate  Marks </td>
		    <td><input type="radio" name="radPer10" id="radPer10" value="1"  <?=$results10[0]->e_percentage_type==1?'checked':''?>/>Percentage&nbsp;&nbsp; <input type="radio" name="radPer10" id="radPer102" value="2" <?=$results10[0]->e_percentage_type==2?'checked':''?>/>CGPA(out of 10 points) <span><font color="#FF0000" style="margin:32px;">*</font></span></td>
		    </tr>
		  <tr>
		    <td>&nbsp;</td>
		    <td><input type="text" name="txtPer10" maxlength="5" style="height:15px; width:100px;" value="<?=stripslashes($results10[0]->e_percentage)?>" /> <span><font color="#FF0000" style=" margin-left:197px;">*</font></span></td>
		    </tr>
		  <tr>
		    <td>School/College </td>
		    <td><input type="text" name="txtSchool10" id="txtSchool10"   style="width:300px; height:20px; border:1px solid #999; background-image:url(images/innertab_text.png); background-repeat:repeat-x;" value="<?=stripslashes($results10[0]->e_institute)?>"/> <span><font color="#FF0000">*</font></span></td>
		    </tr>
		  <tr>
		    <td>Board </td>
		    <td><input type="text" name="txtBoard10" id="txtBoard10"  style="width:300px; height:20px; border:1px solid #999; background-image:url(images/innertab_text.png); background-repeat:repeat-x;" value="<?=stripslashes($results10[0]->e_university)?>"/> <span><font color="#FF0000">*</font></span></td>
		    </tr>
		  <!--<tr>
		    <td valign="top">Address </td>
		    <td><textarea  name="areaAddress"   style="width:300px; height:70px; border:1px solid #999; background-image:url(images/innertab_text.png); background-repeat:repeat-x;resize:none;"><?=stripslashes($results10[0]->e_address);?></textarea></td>
		    </tr>-->
		  <tr><td>&nbsp;</td><td><? if(count($results10)==0){?><input type="submit" name="submit10" value="" class="button_new" />&nbsp;&nbsp;&nbsp;<input type="reset" name="reset" value=""  class="resetbutton" /><? }else{?><input type="submit" name="update10" value=""  class="button_new" /><? }?><span style="font-size:10px; font-family:Trebuchet MS; color:#1C94C4; padding-left:30px;">Save this section before filling up next.</span></td></tr></table></form></p>
	</div>
	<div id="tabs-2">
		    <?  $results12=$db->getResults("select * FROM $education_table where m_id='".$_SESSION[m_id]."' and qua_id='4' limit 1"); ?>
		<p><strong>10+2 Details</strong>
        ...
        <table width="100%" cellpadding="3" cellspacing="0" border="0">
		  ...
		  <tr>
		    <td width="21%">Passed Out </td>
		    <td width="79%"><select name="selMonth12" id="selMonth12" style="width:80px;">
           
            <? for($m=0;$m<count($month_array);$m++){?>
            <option value="<?=$m?>"  <?=$results12[0]->e_passout_month==$m?'selected':''?>><?=$month_array[$m]?></option>
           
            <? }?></select> 
            &nbsp;&nbsp;&nbsp;&nbsp;<select name="selYear12" id="selYear12" style="width:80px;">
            <option value="0">Year</option>
            <? for($y=1990;$y<=date(Y);$y++){?>
            <option value="<?=$y?>"  <?=$results12[0]->e_passout_year==$y?'selected':''?>><?=$y?></option>
		  ...
		  <tr>
		    <td>Aggregate  Marks </td>
		    <td><input type="radio" name="radPer12" id="radPer12" value="1"  <?=$results12[0]->e_percentage_type==1?'checked':''?>/>Percentage&nbsp;&nbsp; <input type="radio" name="radPer12" id="radPer122" value="2" <?=$results12[0]->e_percentage_type==2?'checked':''?>/>CGPA(out of 10)<span><font color="#FF0000" style="margin:78px;">*</font></span></td>
		    </tr>
		  <tr>
		    <td>&nbsp;</td>
		    <td><input type="text" name="txtPer12" maxlength="5" style="height:15px; width:100px;" value="<?=stripslashes($results12[0]->e_percentage)?>" /> <span><font color="#FF0000" style=" margin-left:198px;">*</font></span></td>
		    </tr>
		  <tr>
		    <td>School/College </td>
		    <td><input type="text" name="txtSchool12" id="txtSchool12"  style="width:300px; height:20px; border:1px solid #999; background-image:url(images/innertab_text.png); background-repeat:repeat-x;" value="<?=stripslashes($results12[0]->e_institute)?>"/><span><font color="#FF0000">&nbsp;*</font></span></td>
		    </tr>
		  <tr>
		    <td>Board </td>
		    <td><input type="text" name="txtBoard12" id="txtBoard12" style="width:300px; height:20px; border:1px solid #999; background-image:url(images/innertab_text.png); background-repeat:repeat-x;" value="<?=stripslashes($results12[0]->e_university)?>"/><span><font color="#FF0000">&nbsp;*</font></span></td>
		    </tr>
		  <!--<tr>
		    <td valign="top">Address </td>
		    <td><textarea name="areaAddress12"  style="width:300px; height:70px; border:1px solid #999; background-image:url(images/innertab_text.png); background-repeat:repeat-x;resize:none;"><?=stripslashes($results12[0]->e_address);?></textarea></td>
		    </tr>-->
		  <tr><td>&nbsp;</td><td><? if(count($results12)==0){?><input type="submit" name="submit12" value=""  class="button_new" />&nbsp;&nbsp;&nbsp;<input type="reset" name="reset" value=""  class="resetbutton" /><? }else{?><input type="submit" name="update12" value=""  class="button_new" /><? }?><span style="font-size:10px; font-family:Trebuchet MS; color:#1C94C4; padding-left:30px;">Save this section before filling up next.</span></td></tr></table></form></p>

	</div>
	<div id="tabs-3">
   
		<p><strong>Graduation / Diploma Certificate Course Details</strong>
...
<table width="100%" cellpadding="3" cellspacing="0" border="0">
		  ...
		  <tr>
		    <td valign="top">Country </td><? $value=$resultsGrad[0]->e_country;
           // echo $value;
			if(is_numeric($value))
			{
?><td valign="top"><select name="selCountryGrad" id="selCountryGrad"  style="width:150px;" onChange="return instGrad(this.value);" >
              <option value="99" <?php if ($value == 99) { echo ' selected="selected"'; } ?> >INDIA</option>
              <option value="100" <?php if ($value == 100) { echo ' selected="selected"'; } ?>>USA</option>
              <option value="101"<?php if ($value == 101) { echo ' selected="selected"'; } ?>>UK</option>
              <option value="102"<?php if ($value == 102) { echo ' selected="selected"'; } ?>>Australia</option>
              <option value="103"<?php if ($value == 103) { echo ' selected="selected"'; } ?>>Singapore</option>
 ...
 <input type="text" name="sel_CountryGrad" maxlength="150"  value="" id="divGradtext"  style="height:15px; margin-top:5px; width:147px;display:<?=$value=='-1'?'block':'none'?>;"/>            
		...
		<?	}else
		  ...
		  <tr>
		    ...
		    <td valign="top"><select name="selStateGrad" id="selStateGrad"  style="width:150px;" >
		...
		<textarea name="selInstGrad" value=""  id="selInstGrad" style="width:147px; height:40px;" > <?=stripslashes(trim($resultsGrad[0]->e_institute));?></textarea>
...
<?php /*?><select name="selInstGrad" id="selInstGrad"  style="width:150px;" onChange="return instGrad(this.value);">
	...
	<?php */?><td valign="top"><span style="color:#FF0000;">*</span></td>
		  ...
		  <tr>
		    <td width="26%">Passed Out:</td>
		    <td width="42%"><select name="selMonthGrad" id="selMonthGrad" style="width:80px;">
              <? for($m=0;$m<count($month_array);$m++){?>
		    ...
		    <td width="32%"><span style="color:#FF0000;">*</span></td>
		    </tr>
		  <tr>
		  ...
		  <tr><td valign="top">&nbsp;</td>
		    <td colspan="2" valign="top"><? if(count($resultsGrad)==0){?>
		      ...
		      &nbsp;&nbsp;&nbsp;
		      <input type="reset" name="reset" value="" class="resetbutton"  />
		      <? }else{?>
		      <input type="submit" name="updateGrad" value=""  class="button_new" />
		      <? }?><span style="font-size:10px; font-family:Trebuchet MS; color:#1C94C4; padding-left:30px;">Save this section before filling up next.</span></td>
	    </tr></table>
    ...
    <div id="tabs-4">
		<p><strong>PG / PG Diploma Details</strong>
        ...
        <table width="100%" cellpadding="3" cellspacing="0" border="0">
		  ...
		  <tr>
		     <td valign="top">Branch of Study </td>
		     <td valign="top"><div id="divPGBranch"><select name="selBranchPG" id="selBranchPG"  style="width:150px;"><option value="0">Select Branch</option>
              ...
              <? if(count($resultsPG)!=0){
			   ...
			   ?>
            <option value="<?=$branchPG->branch_id?>" <?=$resultsPG[0]->e_branch==$branchPG->branch_id?'selected':''?>><?=$branchPG->branch_name;?></option>
            <? }}?> 
            </select></div></td>
		     <td valign="top"><span style="color:#FF0000;">*</span></td>
		    </tr>
		  <tr>
             ...
             <td valign="top">Country </td><? $value=$resultsPG[0]->e_country;
			 if(is_numeric($value))
			{?>
 ...
 <input type="text" name="sel_CountryPG" maxlength="150"  value="<?=$value?>" id="divPGtext"   style="height:15px; margin-top:5px; width:147px;"/>            
		  ...
		  <tr>
		     <td valign="top">State </td>
		     <td valign="top"><select name="selStatePG" id="selStatePG"  style="width:150px;">
		      <option value="0">Select State</option>
              <? 
			...
			?>
		     ...
		     <td valign="top">Institute  </td>
		     <td valign="top">
			 <textarea name="selInstPG" value=""  id="selInstPG" style="width:147px; height:40px;" > <?=stripslashes($resultsPG[0]->e_institute);?></textarea>
            ...
            </select>		<input type="text" name="txtotherPGUniv" maxlength="250"  value="<?=$resultsPG[0]->e_other_university?>" id="divPGUnivtext"  style="height:15px; margin-top:5px; width:147px;display:<?=$resultsPG[0]->e_university=='-1'?'block':'none'?>;"/>    </td>
		     <td valign="top"><span style="color:#FF0000;">*</span></td>
            </tr>
		  <tr>
		    <td width="31%">Passed Out:</td>
		    <td width="46%"><select name="selMonthPG" id="selMonthPG" style="width:80px;">
              <? for($m=0;$m<count($month_array);$m++){?>
...
<? }?><span style="font-size:10px; font-family:Trebuchet MS; color:#1C94C4; padding-left:30px;">Save this section before filling up next.</span></td>
	...
	<div id="tabs-5">
		<p><strong>PG Diploma and Certificate courses</strong>
        ...
        <table width="100%" cellpadding="3" cellspacing="0" border="0">
		  ...
		  <tr>
		    <td valign="top">Branch of Study </td>
		    <td valign="top"><div id="loadPGDipBranch"><select name="selBranchPGDip" id="selBranchPGDip"  style="width:150px;"><option value="0">Select Branch</option>
              ...
              <? 
			   ...
			   ?>
            <option value="<?=$branchPGDip->branch_id?>" <?=$resultsPGDip[0]->e_branch==$branchPGDip->branch_id?'selected':''?>><?=$branchPGDip->branch_name;?></option>
            <? }?> 
            </select></div></td>
		    <td valign="top">&nbsp;</td>
		    </tr>
		  <tr>
           <td valign="top">Country </td><? $value=$resultsPGDip[0]->e_country;
		    if(is_numeric($value))
			{?>
 ...
 <input type="text" name="sel_CountryPGDip" maxlength="150"  value="<?=$value?>" id="divPGDiptext"   style="height:15px; margin-top:5px; width:147px;"/>            
            </td>
            <? }?> 
		  ...
		  <tr>
		    <td valign="top">State </td>
		    <td valign="top"><select name="selStatePGDip" id="selStatePGDip"  style="width:150px;">
		      <option value="0">Select State</option>
              <? 
		    ...
		    <td valign="top">
			 ...
			 <textarea name="selInstPGDip" value=""  id="selInstPGDip" style="width:147px; height:40px;" > <?=stripslashes(trim($resultsPGDip[0]->e_institute));?></textarea>
		    ...
		    <?php */?><td valign="top"><span style="color:#FF0000;">*</font></span></td>
		    </tr><?php /*?><tr>
		    <td valign="top">University   </td>
		  ...
		  <tr>
		    ...
		    <td width="23%"><span style="color:#FF0000;">*</font></span></td>
		    </tr>
		  <tr>
		    <td valign="top">Aggregate  Marks</td>
...
<? }?><span style="font-size:10px; font-family:Trebuchet MS; color:#1C94C4; padding-left:30px;">Save this section before filling up next.</span></td>
    ...
    <div id="tabs-6" style="overflow:scroll; height:400px; display:none;">
      ...
      <table width="100%" cellpadding="3" cellspacing="0" border="0">
		  <tr>
		    <td valign="top">&nbsp;</td>
		    <td width="47%" class="error"><?=messaging($_REQUEST[msg])?></td>
		  ...
		  <tr>
		    <td valign="top">Branch of Study </td>
		    <td valign="top"><div id="divOtherBranch<?=$resultsOther->e_id?>"><select name="selBranchOther" id="selBranchOther"  style="width:150px;"><option value="0">Select Branch</option>
              ...
              <? if(count($resultsOther)!=0){
			   ...
			   ?>
            <option value="<?=$branchOther->branch_id?>" <?=$resultsOther->e_branch==$branchOther->branch_id?'selected':''?>><?=$branchOther->branch_name;?></option>
            <? }}?> 
            </select></div></td>
		    <td valign="top">&nbsp;</td>
		    </tr>
		  ...
		  <tr>
		    <td valign="top">State </td>
		    <td valign="top"><select name="selStateOther" id="selStateOther"  style="width:150px;">
		      <option value="0">Select State</option>
              <? 
            ...
            </select>	 <input type="text" name="txtotherUniv" maxlength="250"  value="<?=$resultsOther->e_other_university?>" id="divOtherUnivtext<?=$resultsOther->e_id?>"  style="height:15px; margin-top:5px; width:147px;display:<?=$resultsOther->e_university=='-1'?'block':'none'?>;"/>	    </td>
		    <td valign="top"><span style=" color:#FF0000;">*</span></td>
		    </tr>
		  <tr>
...
<div style="text-align:right;"><a href="academic_info.php" ><img src="images/continue.png" border="0" /></a>
