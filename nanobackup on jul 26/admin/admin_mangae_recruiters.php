...
include_once('../classes/dataBase.php');
   ...
   include_once('../classes/inputFields.php');
	...
	//include_once('../classes/recruiter.class.php');  
	  ...
	  $deletetemprid = mysql_query("Delete from tbl_temprid where sessionid = '$sessionid'");
...
;
	...
	while ($row = mysql_fetch_assoc($resultc)) {
		...
		$tempidsessions = "SELECT DISTINCT (
r_id
)
FROM  `rv_job_posting` 
WHERE  `upddate` !=  '$dates'
...
$resultsessions = mysql_query($tempidsessions);
while($row = mysql_fetch_assoc($resultsessions))
{
  
   $rid=$row['r_id'];
   $prid = $prid.','.$row['r_id'];
   $selectrecruiterpending = "Select * from rv_recruiters where r_id=$rid";
		
		$executerecruiterpending = mysql_query($selectrecruiterpending);
		//$cntpending = count($executeselectjobpending);
			while ($row = mysql_fetch_assoc($executerecruiterpending)) {
			$r_user_name	= $row["r_user_name"];
		$r_company	= $row["r_company"];
		$r_mobile	= $row["r_mobile"];
		$r_email	= $row["r_email"];
		$r_designation	= $row["r_designation"];
		$r_active	= $row["r_active"];
		$idrecruiterjobpending  = $r_id	= $row["r_id"];
		$registereddate	= $row["registereddate"];
			}
  $sessionid;
  $Insert	=	mysql_query("INSERT INTO tbl_temprid(r_id,sessionid,r_user_name,r_company,r_mobile,r_email
  ,r_designation,r_active,registereddate)
							VALUES ('".$rid."','".$sessionid."','".$r_user_name."','".$r_company."','".$r_mobile."','".$r_email."'
,'".$r_designation."','".$r_active."','".$registereddate."')");
...
$s=0;
$resultoldjobs = "Select a.* from tbl_temprid as a where a.sessionid='$sessionid'";
