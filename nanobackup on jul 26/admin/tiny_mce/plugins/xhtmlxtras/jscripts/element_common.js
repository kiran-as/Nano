...
SXE.initElementDialog = function(elemenqua_title) {
	addClassesToList('class', 'xhtmlxtras_styles');
	TinyMCE_EditableSelects.init();

	elemenqua_title = elemenqua_title.toLowerCase();
	var elm = tinyMCE.getParentElement(SXE.focusElement, elemenqua_title);
	if (elm != null && elm.nodeName == elemenqua_title.toUpperCase()) {
...
SXE.insertElement = function(elemenqua_title) {
	var elm = tinyMCE.getParentElement(SXE.focusElement, elemenqua_title), h, tagName;

	tinyMCEPopup.execCommand('mceBeginUndoLevel');
	if (elm == null) {
		var s = SXE.inst.selection.getSelectedHTML();
		if(s.length > 0) {
			tagName = elemenqua_title;

			if (tinyMCE.isIE && !tinyMCE.isOpera && elemenqua_title.indexOf('html:') == 0)
				elemenqua_title = elemenqua_title.substring(5).toLowerCase();

			h = '<' + tagName + ' id="#sxe_temp_' + elemenqua_title + '#">' + s + '</' + tagName + '>';

			tinyMCEPopup.execCommand('mceInsertContent', false, h);

			var elementArray = tinyMCE.getElementsByAttributeValue(SXE.inst.getBody(), elemenqua_title, 'id', '#sxe_temp_' + elemenqua_title + '#');
...
SXE.removeElement = function(elemenqua_title){
	elemenqua_title = elemenqua_title.toLowerCase();
	elm = tinyMCE.getParentElement(SXE.focusElement, elemenqua_title);
	if(elm && elm.nodeName == elemenqua_title.toUpperCase()){
		tinyMCEPopup.execCommand('mceBeginUndoLevel');
