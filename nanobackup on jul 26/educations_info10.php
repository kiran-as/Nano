...
/*  if($_POST)
  ...
  if(isset($_POST['hidId10']))
  {
       ...
       $db->insertRecord("update  $education_table  set 
		...
		header("Location:educations_info10.php?msg=0&tb=2#tabs-2");
  ...
  if(isset($_POST[submit10])){
	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
	    ...
	    header("Location:educations_info10.php?msg=0&tb=1#tabs-1");
	  }
...
if(isset($_POST[submit12])){
	...
	$typedip = 1;
	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
															...
															e_address		='".$ch->checkFields($_POST[areaAddress]) ."',
															e_school_percentage='".$typedip."',
															e_sylbus='".$ch->checkFields($_POST[pudetails])."',
															e_agg_marks		='".$ch->checkFields($_POST[txtdiplomaBoard12]) ."',
															e_percentage	='".$ch->checkFields($_POST[txtPer12]) ."'");
	  $typedip = 2;
	  $coursedip =55;
	   if($ch->checkFields($_POST[txtdiploma12])!=0)
	 {
	  	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
															e_institute		='".$ch->checkFields($_POST[txtdiploma12]) ."',
															e_university	='".$ch->checkFields($_POST[txtdiplomaBoard12]) ."',
															e_passout_year	='".$ch->checkFields($_POST[selYear121]) ."',
															e_percentage_type='".$ch->checkFields($_POST[radPer121]) ."',
														e_course='".$coursedip."',
															e_school_percentage='".$typedip."',
														
															e_percentage	='".$ch->checkFields($_POST[txtPer121]) ."'");
	 }
	 else 
	 {
	    ...
	    {
	    	abcmailfunction($currentempfctr);
			  header("Location:educations_info10.php?msg=0&tb=3#tabs-3");
	    }
	    header("Location:educations_info10.php?msg=0&tb=3#tabs-3");
	  }
...
$result	=	mysql_query("delete from $education_table where qua_id=4 and m_id=".$_SESSION[m_id]);
	 ...
	 $typedip = 1;
	  ...
	  $result	=	mysql_query("delete from $education_table where e_course='55' and m_id=".$_SESSION[m_id]);
	...
	else 
	{
		...
		/*echo "insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
$															e_institute		='".$ch->checkFields($_POST[txtdiploma12]) ."',
															e_university	='".$ch->checkFields($_POST[txtdiplomaBoard12]) ."',
															e_passout_year	='".$ch->checkFields($_POST[selYear121]) ."',
															e_percentage_type='".$ch->checkFields($_POST[radPer121]) ."',
														e_course='".$coursedip."',
															...
															e_percentage	='".$ch->checkFields($_POST[txtPer121]) ."'";die();*/
	 	 $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
															e_institute		='".$ch->checkFields($_POST[txtdiploma12]) ."',
															e_university	='".$ch->checkFields($_POST[txtdiplomaBoard12]) ."',
															e_passout_year	='".$ch->checkFields($_POST[selYear121]) ."',
...
$course = $ch->checkFields($_POST[selCourseGrad]);
if($course=='53')
{
  $type=53;
  $course=25;
...
$school = 1;
	 $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
															...
															e_type	='".$type."',
															e_sylbus	='".$ch->checkFields($_POST[radPerGrad1]) ."',
															e_agg_marks	='".$ch->checkFields($_POST[selYearGrad1]) ."',
															e_school_percentage='".$school."',
															e_percentage	='".$ch->checkFields($_POST[txtPerGrad]) ."'");
	 
	  if($ch->checkFields($_POST[selInstGrad1])=='0')
	 {
	...
	else 
	{
	 ...
	 $course = 16;
	   $type=25;
	 	 ...
	 	 $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
															qua_id			='".$ch->checkFields(2) ."',
															e_course		='".$course."',
															e_branch		='".$ch->checkFields($_POST[selBranchGrad1]) ."',
															e_institute		='".$ch->checkFields($_POST[selInstGrad1]) ."',
															e_university	='".$ch->checkFields($_POST[selUniversityGrad1]) ."',
															e_passout_year	='".$ch->checkFields($_POST[selYearGrad1]) ."',
															e_percentage_type='".$ch->checkFields($_POST[radPerGrad1]) ."',
															e_school_percentage='".$school."',
															e_percentage	='".$ch->checkFields($_POST[txtPerGrad1]) ."'");
	 ...
	 $currentempfctr = (employabilityFactor($_SESSION[m_id]));
	    ...
	    {
	    	abcmailfunction($currentempfctr);
			  header("Location:educations_info10.php?msg=0&tb=4#tabs-4");
	    }
	   header("Location:educations_info10.php?msg=0&tb=4#tabs-4");
	  }
...
if(isset($_POST[updateGrad])){
	...
	else
	{
		$grad=$ch->checkFields($_POST[selCountryGrad]);
		  ...
		  $result	=	mysql_query("delete from $education_table where qua_id='2' and m_id=".$_SESSION[m_id]);
...
if(isset($_POST[PGSubmit])){
	...
	else
	{
	  ...
	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
	    ...
	    {
	    	abcmailfunction($currentempfctr);
			  header("Location:educations_info10.php?msg=0&tb=5#tabs-5");
	    }
	    header("Location:educations_info10.php?msg=0&tb=5#tabs-5");
	  }
...
if(isset($_POST[updateOther])){
	  $db->insertRecord("update  $education_table  set 
	     ...
	     header("Location:educations_info10.php?msg=2&tb=6#tabs-6");
	  }	
if(isset($_POST[OthermoreSubmit])){
	  ...
	  $db->insertRecord("insert into  $education_table  set m_id			='".$ch->checkFields($_SESSION[m_id]) ."',
	    ...
	    header("Location:educations_info10.php?msg=2#tabs-6");
	  }
...
if($_REQUEST[action]=='deleteOne' && is_numeric($_REQUEST[e_id])!=''){
 ...
 header("Location:educations_info10.php?msg=3&tb=6#tabs-6");
}
...
$uni_query=$db->getResults("select * FROM $universities_table order by uni_name asc"); 
 ...
 $results12=$db->getResults("select * FROM $education_table where m_id='".$_SESSION[m_id]."' and e_school_percentage=1 limit 1");
$agg = $results12[0]->e_sylbus;

 $results13=$db->getResults("select * FROM $education_table where m_id='".$_SESSION[m_id]."' and qua_id='2' and e_school_percentage=1 limit 1");
$agg13 = $results13[0]->e_type;
//print_r($results13);
//print_r($agg13);
 ?>
...
function tenthdiploma()
{
	var aggmarks = '<?php echo $agg;?>';
	if(aggmarks=='10')
	{
		enterdetails(10);
	}
	...
	var aggmarks13 = '<?php echo $agg13;?>';
	if(aggmarks13!='')
	{
		hideshows(<?php echo $agg13;?>);
	}
	else
	{
		hideshows(0);
...
/*
     function fnshow2()
	 {
    	 ...
    	 {
        	 alert('falseaaaa');
    		 header("Location:1_educations_info10.php?msg=0&tb=1#tabs-1");
    	 }
    	 if(flag == true)
    	 {
        	 alert('asfsdasdf');
...
-->
  ...
  <div class="demo" style="float:left; top:20px; height:650px;padding:10px;">
    <div id="tabs">
...
function skiptofifthtab()
{
	...
	window.location="http://nanochipsolutions.com/educations_info10.php?msg=0&tb=5#tabs-5";
...
function enterdetails(detail)
{

	 if(detail==10)
	 {
		 document.getElementById('12details').style.display='none';
		 document.getElementById('14details').style.display='none';
		 document.getElementById('10details').style.display='';
		 document.getElementById('10details1').style.display='';
		 document.getElementById('10details2').style.display='none';
		 document.getElementById('10details3').style.display='none';
		 ...
		 document.getElementById('diploma4').style.display='none';	
		 document.getElementById('diploma5').style.display='none';	
		 document.getElementById('txtdiploma12').value='0';
	 		 
	 }
	 else if(detail==12)
	 {
		 document.getElementById('12details').style.display='';
		 document.getElementById('14details').style.display='none';
		 document.getElementById('10details').style.display='none';	
		 document.getElementById('10details1').style.display='none';
		 document.getElementById('10details2').style.display='';
		 document.getElementById('10details3').style.display='none';
		 document.getElementById('diploma1').style.display='none';
		 document.getElementById('diploma2').style.display='none';
		 document.getElementById('diploma3').style.display='none';
	 ...
	 {
		 document.getElementById('12details').style.display='none';
		 document.getElementById('14details').style.display='';
		 document.getElementById('10details').style.display='none';
		 document.getElementById('10details1').style.display='';
		 document.getElementById('10details2').style.display='none';	
		 document.getElementById('10details3').style.display='';
		 document.getElementById('diploma1').style.display='';
		 document.getElementById('diploma2').style.display='';	
		 document.getElementById('diploma3').style.display='';
		 document.getElementById('diploma4').style.display='';	
		 document.getElementById('diploma5').style.display='';			 
	 }
}

function hideshows(id)
{
	
	if(id=='0')
	{
		 document.getElementById('bedetails').style.display='none';
		 document.getElementById('bscdetails').style.display='none';
		 document.getElementById('bscheading').style.display='none';
		 document.getElementById('bscdetailsschool').style.display='none';
		 document.getElementById('bscdetailsschool1').style.display='none';
		 document.getElementById('bscdetailsschool2').style.display='none';
		 document.getElementById('bscdetailsschool3').style.display='none';
		 document.getElementById('bscdetailsschool4').style.display='none';	
		 document.getElementById('bscdetailsschool5').style.display='none';	
		 
 		 document.getElementById('selInstGrad1').value='0';	
	 	 
	}
	else if(id==16)
	{
		 document.getElementById('bedetails').style.display='none';
		 document.getElementById('bscdetails').style.display='';
		 document.getElementById('bscheading').style.display='none';
		 document.getElementById('bscdetailsschool').style.display='none';
		 document.getElementById('bscdetailsschool1').style.display='none';
		 document.getElementById('bscdetailsschool2').style.display='none';
	...
	{
		 document.getElementById('bedetails').style.display='';
		 document.getElementById('bscdetails').style.display='none';
		 document.getElementById('bscheading').style.display='';
		 document.getElementById('bscdetailsschool').style.display='';
		 document.getElementById('bscdetailsschool1').style.display='';
		 document.getElementById('bscdetailsschool2').style.display='';
		 document.getElementById('bscdetailsschool3').style.display='';
		 document.getElementById('bscdetailsschool4').style.display='';
		 document.getElementById('bscdetailsschool5').style.display='';			 
	...
	else
	{
      ...
      <div id="tabs-2">
        <?  $results12=$db->getResults("select * FROM $education_table where m_id='".$_SESSION[m_id]."' and qua_id='4' and e_school_percentage=1 "); ?>
        <p><strong id="10details">10+2 Details</strong>
        <strong id="12details" style="display:none">Diploma</strong>
        <strong id="14details" style="display:none">10+2 & Diploma Details</strong> 
        </p>
        <form name="form12" id="form12" method="post" action="" onsubmit="return class12thvalidation();">
          <table width="100%" cellpadding="3" cellspacing="0" border="0">
            <tr>
             ...
             <tr>
              <td class="label" nowrap="nowrap" align="right">Aggregate  Marks<span><font color="#FF0000">*</font></span></td>
              <td>            <select name="pudetails" id="pudetails"  class="label" nowrap="nowrap" style="width:250px;" onchange="enterdetails(this.value)" >
                  <option value="10" <?=$results12[0]->e_sylbus==10?'selected':''?>>10+2 Details</option>
                  <option value="12" <?=$results12[0]->e_sylbus==12?'selected':''?>>Diploma Details</option>
                  <option value="14" <?=$results12[0]->e_sylbus==14?'selected':''?>>10+2 & Diploma Details</option></select></td>
            </tr>
            <tr>
              <td colspan="2" align="left"><strong id="10details1">Enter Your 10+2 Details</strong>
              <strong id="10details2" style="display:none">Enter Your Diploma Details</strong>
            ...
            <input type="hidden" name="hidId12" value="<?=$results12[0]->e_id?>" />
             <input type="hidden" name="school" value="<?=$results12[0]->e_id?>" />
            ...
            <tr>
              ...
              <strong id="10details3" style="display:none">Enter Your Diploma Details</strong>
                 <?  $resultsdip=$db->getResults("select * FROM $education_table where m_id='".$_SESSION[m_id]."'  and e_school_percentage=2 "); ?>
              </td>
               <tr id='diploma5' style="display:none">
              <td class="label" nowrap="nowrap" align="right">Passed Out<span><font color="#FF0000">*</font></span></td>
              <td>
                <select name="selYear121" id="selYear121"  class="label" nowrap="nowrap" style="width:250px;" >
                  <option value="0">Year</option>
                  <? for($y=1990;$y<=date(Y);$y++){?>
                  <option value="<?=$y?>"  <?=$resultsdip[0]->e_passout_year==$y?'selected':''?>>
                    <?=$y?>
                    </option>
                  <? }?>
                </select>
              ...
              <tr id='diploma3' style="display:none">
              <td class="label" nowrap="nowrap" align="right">Aggregate  Marks<span><font color="#FF0000">*</font></span></td>
              <td><input type="radio" name="radPer121" id="radPer12" value="1"  <?=$resultsdip[0]->e_percentage_type==1?'checked':''?>/>
                Percentage&nbsp;&nbsp;
                <input type="radio" name="radPer121" id="radPer122" value="2" <?=$resultsdip[0]->e_percentage_type==2?'checked':''?>/>
                CGPA(out of 10)</td>
            </tr>
            <tr id='diploma4' style="display:none">
              <td></td>
              <td><input type="text" name="txtPer121" id="txtPer121" maxlength="5" class="label" nowrap="nowrap" style="width:250px;" value="<?=stripslashes($resultsdip[0]->e_percentage)?>" />
                </td>
            </tr>
            
            <tr id='diploma1' style="display:none">
              <td class="label" nowrap="nowrap" align="right">School/College<span><font color="#FF0000">*</font></span></td>
              <td><input type="text" name="txtdiploma12" id="txtdiploma12" class="label" nowrap="nowrap" style="width:250px;" value="<?=stripslashes($resultsdip[0]->e_institute)?>"/>
                  </td>
            </tr>
            <tr id='diploma2' style="display:none">
              <td class="label" nowrap="nowrap" align="right">Board<span><font color="#FF0000">*</font></span></td>
              <td><input type="text" name="txtdiplomaBoard12" id="txtdiplomaBoard12" class="label" nowrap="nowrap" style="width:250px;" value="<?=stripslashes($resultsdip[0]->e_university)?>"/>
                  </td>
            </tr>
      ...
      <div id="tabs-3">
        ...
        <form name="formGrad" id="formGrad" method="post" action="" onsubmit="return Graduationvalidation();">
          <?  $resultsGrad=$db->getResults("select * FROM $education_table where m_id='".$_SESSION[m_id]."' and qua_id='2' and  e_school_percentage=1 ");
		  ?>
          <table width="100%" cellpadding="3" cellspacing="0" border="0">
            <tr>
              <td>&nbsp;</td>
            ...
            <tr>
              ...
              <td><select name="selCourseGrad" id="selCourseGrad" class="label" nowrap="nowrap" style="width:250px;" onblur="hideshows(this.value)" onchange="loadGradBranch(this.value);return false;">
			   ...
			   ?>
                  <option value="<?=$courseGrad->cor_id?>" <?=$resultsGrad[0]->e_type==$courseGrad->cor_id?'selected':''?>>
                  <?=$courseGrad->cor_name;?>
                  </option>
                  <? }?>
                </select>
             ...
             <tr>
              <td colspan="2" align="left"><strong id="bedetails">Enter Your B.E Details</strong>
              <strong id="bscdetails" style="display:none">Enter Your Bsc Details</strong>
            ...
            <tr>
              ...
              <td><div id="divGradBranch">
                  <select name="selBranchGrad" id="selBranchGrad"  class="label" nowrap="nowrap" style="width:250px;">
                    ...
                    <? 
				  $branch_query=$db->getResults("select * from rv_branch where  branch_status!=0 and cor_id in (16,25) order by branch_name asc");
...
?>
	...
	<?php */?></td>
            ...
            <tr>
             ...
             <tr id='bscheading'>
              <td colspan="2" align="left">
              <strong >Enter Your Bsc Details</strong>
                        <?  $resultsGradbsc=$db->getResults("select * FROM $education_table where m_id='".$_SESSION[m_id]."' and qua_id='2' and  e_school_percentage=2"); ?>
              </td>
            </tr>
			 <tr id='bscdetailsschool5' style="display:none">
			 <td class="label" nowrap="nowrap" align="right">Branch of Study<span  style="color:#FF0000;">*</span></td>
              <td><div id="divGradBranch">

                   <select name="selBranchGrad1" id="selBranchGrad1"  class="label" nowrap="nowrap" style="width:250px;" >
                  <option value="68" <?=$resultsGradbsc[0]->e_branch==68?'selected':''?>>Computer Science</option>
                  <option value="73" <?=$resultsGradbsc[0]->e_branch==73?'selected':''?>>Electronics / Applied Electronics</option>
                   </select>
              </div></td>
                   <td></td>
			</tr>			   
            <tr id='bscdetailsschool' style="display:none">
              <td class="label" nowrap="nowrap" align="right">Institute  /College <span style="color:#FF0000;">*</span></td>
              <td><textarea name="selInstGrad1" value=""  id="selInstGrad1" class="label" nowrap="nowrap" style="width:250px; height:40px;" > <?=stripslashes(trim($resultsGradbsc[0]->e_institute));?>
  </textarea>
                  <?php /*?><select name="selInstGrad" id="selInstGrad"  style="width:150px;" onChange="return instGrad(this.value);">
              <option value="0">Select Institute</option>
              <? 
	...
	<?php */?></td>
           ...
           <tr id='bscdetailsschool1' style="display:none">
              <td class="label" nowrap="nowrap" align="right">Passed Out<span style="color:#FF0000;">*</span></td>
              <td ><?php /*?><select name="selMonthGrad" id="selMonthGrad" style="width:80px;">
                  <? for($m=0;$m<count($month_array);$m++){?>
                  <option value="<?=$m?>" <?=$resultsGrad[0]->e_passout_month==$m?'selected':''?>>
                  ...
                  <select name="selYearGrad1" id="selYearGrad1" style="width:250px;">
                    <option value="0">Year</option>
                    <? for($y=1990;$y<=date(Y);$y++){?>
                    <option value="<?=$y?>" <?=$resultsGradbsc[0]->e_passout_year==$y?'selected':''?>>
                    <?=$y?>
                    </option>
                    <? }?>
                </select></td>
              <td width="29%"></td>
                   ...
                   <tr id='bscdetailsschool4' style="display:none">
              <td class="label" nowrap="nowrap" align="right">University<span style="color:#FF0000;">*</span></td>
              <td><select name="selUniversityGrad1" id="selUniversityGrad1" class="label" nowrap="nowrap" style="width:250px;" onchange="return univGrad(this.value);">
                  <option value="0">Select University<span style="color:#FF0000;">*</span></option>
                  <? 
			foreach($uni_query as $uni_grad){
			?>
                  <option value="<?=stripslashes($uni_grad->uni_id)?>" <?=$resultsGradbsc[0]->e_university==$uni_grad->uni_id?'selected':''?>>
                  <?=stripslashes($uni_grad->uni_name);?>
                  </option>
                  <? }?>
                  <option value="-1" <?=$resultsGradbsc[0]->e_university=='-1'?'selected':''?>>Other</option>
                </select>
                  <input type="text" name="txtotherGradUniv" maxlength="250"  value="<?=$resultsGradbsc[0]->e_other_university?>" id="divUnivtext"  style="height:15px; margin-top:5px; width:250px;display:<?=$resultsGradbsc[0]->e_university=='-1'?'block':'none'?>;"/></td>
              <td></td>
            </tr> 
            <tr id='bscdetailsschool2' style="display:none">
              <td class="label" nowrap="nowrap" align="right">Aggregate  Marks<span style="color:#FF0000;">*</span></td>
              <td><input type="radio" name="radPerGrad1" id="radPerGrad" value="1"   <?=$resultsGradbsc[0]->e_percentage_type==1?'checked':''?> />
                Percentage&nbsp;&nbsp;
                <input type="radio" name="radPerGrad1" id="radPerGrad2" value="2"  <?=$resultsGradbsc[0]->e_percentage_type==2?'checked':''?> />
                CGPA(out of 10)</td>
              <td></td>
            </tr>
                  <tr id='bscdetailsschool3' style="display:none">
              <td>&nbsp;</td>
              <td><input type="text" id='txtPerGrad1' name="txtPerGrad1" maxlength="5" class="label" nowrap="nowrap" style="width:250px;" value="<?=$resultsGradbsc[0]->e_percentage?>" /></td>
              <td><span style="color:#FF0000;">*</span></td>
            </tr>
