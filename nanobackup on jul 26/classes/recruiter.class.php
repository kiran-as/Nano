...
public function  delRecruiters($r_id)
	...
	public function  getJobApp($count = "",$limitvalue = "",$limit = "",$ja_id="")
	{
		global $job_app_table;
			...
			$db = new dataBase();
			$sel_sql= "select * from rv_job_posting";
			//echo $ja_id;die;
			
			 if(!empty($ja_id))
			 {
			 $sel_sql.=" where jp_id ='".$ja_id."' and r_id='".$_SESSION[r_id]."'";
			 $result=$db->getResults($sel_sql);
			 }
			...
			else
			{
				if($limitvalue!="" or $limit != "")
				{
				$sel_sql.=" order by jp_id desc  LIMIT $limitvalue, $limit";
				}
				//echo $sel_sql;die;
				$result=$db->getResults($sel_sql);
	...
	public function  delJobApp($j_id)
	{
		global $job_app_table;
	
			$db = new dataBase();
			
			$del_sql= "delete from rv_job_posting where jp_id = $j_id";
			
			 return $result=$db->deleteRecord($del_sql);

	}
	
	public function  updateJobapp($j_id,$POST)
	{
	global $job_app_table;
		...
		//print_r($array);die;
		 ...
		 $up_sql= "update rv_job_posting set st_ids='".$array."' where jp_id = $j_id";
		//echo $up_sql;die;
