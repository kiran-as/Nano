...
$resultsss = "SELECT * FROM  tbl_studentdetailsinformation";

	$resultc = mysql_query($resultsss);
	$s=0;
	while ($row = mysql_fetch_assoc($resultc)) {
		  $arraStudent[$s]["StudentName"]	= $row["StudentName"];
		 $arraStudent[$s]["StudentEmail"]	= $row["StudentEmail"];
		 $arraStudent[$s]["StudentPhone"]	= $row["StudentPhone"];
		 $arraStudent[$s]["idstudentdetailsinformation"]	= $row["idstudentdetailsinformation"];
		  $s++;  
		}
...
function deletemanageprog(idprog)
{
 var deletess = confirm("Do you really want to delete");
  if(deletess == true)
  {
	 ...
	 window.location = "deletestudents.php?idstudentdetailsinformation="+idprog;
   }
}
...
function deletemsg()
{
 var deletess = confirm("Do you really want to delete");
  if(deletess == true)
  {
	 ...
	 window.location = "deleteallstudents.php";
   }
}
          ...
          <tr>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td width="332" align="right" valign="bottom" class="link_green" style="padding-bottom:5px;">
					<?php if($_SESSION['rv_vlsi_id']==2){?>
					<a href="logout.php" class="link_green">Logout</a>
						<?php }?>
...
<tr><td width="20%" valign="top">
 ...
 <td align="right">
<a href="importstudents.php" class="blueButton">Export to Excel</a>
