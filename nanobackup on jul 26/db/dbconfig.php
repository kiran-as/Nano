...
/*
$username="nanoscom_newncs";
$password="newncs";
$hostname="localhost";
$database="nanoscom_newncs";
$server_url="http://www.nanochipsolutions.com/";
*/

$username="nanoscom_ncs";
$password="xnLrKEwvt2li";
$hostname="localhost";
$database="nanoscom_ncs";
$server_url="http://www.nanochipsolutions.com/";
...
if($student_10th[0]->e_percentage>=60 && count($student_10th)!=0 && $student_12th[0]->e_percentage>60 && count($student_12th)!=0){
$rv_marks+=5;
}
 ...
 // For BSc
  $student_dep=$db->getResults("select * from rv_educational_details where m_id='".$m_id."' and e_course='16'");

if(count($student_dep)!=0){
$rv_marks+=25;
} 
/* //  For graduation except  BE or B.tach
  $student_graduation=$db->getResults("select * from rv_educational_details where m_id='".$m_id."' and qua_id='2' and e_course!=25 ");

if(count($student_graduation)!=0){
$rv_marks+=25;
}*/   
  //  For graduation ( BE or B.tach)
  $student_be=$db->getResults("select * from rv_educational_details where m_id='".$m_id."' and e_course='25' ");

if(count($student_be)!=0){

// b.tech in NIT,IIT AND ISSC
if($student_be[0]->e_university=='58' || $student_be[0]->e_university=='59' || $student_be[0]->e_university=='60' || $student_be[0]->e_university=='INDIAN INSTITUTE OF TECHNOLOGY' || $student_be[0]->e_university=='NATIONAL INSTITUTE OF TECHNOLOGY' || $student_be[0]->e_university=='NATIONAL INSTITUTE OF SCIENCE'){
if($student_be[0]->e_percentage<60){
$rv_marks+=45;
}else{
$rv_marks+=50;
}
...
//MSc
  $student_ms=$db->getResults("select * from rv_educational_details where m_id='".$m_id."' and e_course='14' ");

if(count($student_ms)!=0){
$rv_marks+=5;
}


//M.E/M.Tech
  $student_me=$db->getResults("select * from rv_educational_details where m_id='".$m_id."' and e_course='1' ");

if(count($student_me)!=0){
$rv_marks+=5;
}

//MS
  $student_ms=$db->getResults("select * from rv_educational_details where m_id='".$m_id."' and e_course='13' ");

if(count($student_ms)!=0){
if($student_ms[0]->e_country=='99'){
$rv_marks+=5;
}else{
$rv_marks+=10;
}
...
if($project_emp_count!=0 && $projects_employee[0]->p_end=='EMBEDED'){
	  $rv_marks+=5;
	  }
...
//PG diploma in RV-VLSI
  
  $student_pgdip=$db->getResults("select * from rv_educational_details where m_id='".$m_id."' and e_course='48' ");

if(count($student_pgdip)!=0){
if($student_pgdip[0]->e_institute=='RV-VLSI Design Center'){
if($student_pgdip[0]->e_percentage>70){
$rv_marks+=30;
}else{
$rv_marks+=25;
}}else{
...
//For Acadimic Projects 3 points  

/*  $acadimic_projects=$db->getResults("SELECT * FROM rv_academic_projects where m_id='".$m_id."'");
  $pr_cnt=count($acadimic_projects);
if($pr_cnt!=0){
$rv_marks+=3*$pr_cnt;
}*/
 
     $projects_employee=$db->getResults("select * from rv_academic_projects where m_id='".$m_id."'");
	  $project_emp_count=count($projects_employee);
...
$totYears=ceil($years1);
if($totYears>=1){
$rv_marks+=25;
$fromsecond=$totYears-1;
$rv_marks+=$fromsecond*5;
}
